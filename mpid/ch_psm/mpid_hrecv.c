...
#include "mpid_pack.h"
#include "psmdev.h"             /* for MPID_PSM_Irecv */
...
static int MPID_IrecvDatatypeFinish(void *handle)
{
    ...
    /* xxx note: the field "start" in the psm implementation is 
                       ...
                       MPI_Status * status, int *error_code)
{
    ...
    *error_code = 0;
    ...
    if (!*error_code) {
      MPID_PSM_RecvComplete((MPIR_RHANDLE *)request);
      if(status)
          *status = rhandle.s;
    }
}
                        ...
                        MPI_Request request, int *error_code)
{
     ...
     * MPID_PSM_Irecv, but in this case we will pass the allocated
    ...
    MPID_PSM_Irecv(mybuf, len, src_lrank, tag, context_id,
