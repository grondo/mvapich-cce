/*
 *  $Id: intra_fns.c,v 1.2 2007/03/02 16:36:54 surs Exp $
 *
...
#define MPIR_ERR_OP_NOT_DEFINED                                     \
MPIR_ERRCLASS_TO_CODE(MPI_ERR_OP,MPIR_ERR_NOT_DEFINED)
...
#endif
    1
        /* Giving it a refcount of 1 ensures it
...
static int intra_Allgather(void *sendbuf,
                           int sendcount,
                           struct MPIR_DATATYPE *sendtype,
                           void *recvbuf,
                           int recvcount,
                           ...
                           struct MPIR_COMMUNICATOR *comm)
{
    ...
    mpi_errno =
        MPI_Sendrecv(sendbuf, sendcount, sendtype->self, rank,
                     MPIR_ALLGATHER_TAG,
                     (void *) ((char *) recvbuf +
                               rank * recvcount * recv_extent), recvcount,
                     recvtype->self, rank, MPIR_ALLGATHER_TAG, comm->self,
                     &status);
    if (mpi_errno)
        return mpi_errno;
    ...
    for (i = 1; i < size; i++) {
        mpi_errno =
            MPI_Sendrecv((void *) ((char *) recvbuf +
                                   j * recvcount * recv_extent), recvcount,
                         recvtype->self, right, MPIR_ALLGATHER_TAG,
                         (void *) ((char *) recvbuf +
                                   jnext * recvcount * recv_extent),
 ...
 */
static int intra_Allreduce(void *sendbuf,
                           void *recvbuf,
                           int count,
                           struct MPIR_DATATYPE *datatype,
                           MPI_Op op, struct MPIR_COMMUNICATOR *comm)
