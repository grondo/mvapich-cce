...
#define PARAMFILE_LEN 256
    ...
    if (hostfile_on) {
        ...
        for (i = 0; i < nprocs; i++) {
            plist[i].hostname = (char *)malloc(hostname_len);
            gethostname(plist[i].hostname, hostname_len);
...
void usage(void)
{
    ...
    fprintf(stderr, "or\thostfile   => "
            "name of file containing hosts, one per line. If not specified it tries to have the hostfile "
