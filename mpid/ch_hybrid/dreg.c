...
static inline vma_t *vma_new (unsigned long start, unsigned long end)
{
	...
	if (mvparams.dreg_cache_limit &&
			pinned_pages_count + (end - start + 1) > 
				mvparams.dreg_cache_limit)
		return NULL;
...
void dreg_init()
{
    ...
    dreg_free_list = (dreg_entry *)
        malloc(sizeof(dreg_entry) * mvparams.ndreg_entries);

    if (dreg_free_list == NULL) {
	   error_abort_all(GEN_EXIT_ERR,
                            "dreg_init: unable to malloc %d bytes",
                            (int) sizeof(dreg_entry) * mvparams.ndreg_entries);
    }

    for (i = 0; i < (int) mvparams.ndreg_entries - 1; i++) {
        dreg_free_list[i].next = &dreg_free_list[i + 1];
    }

    dreg_free_list[mvparams.ndreg_entries - 1].next = NULL;
    ...
    deregister_mr_array = (dreg_region *)
        malloc(sizeof(dreg_region) * mvparams.ndreg_entries);

    if(NULL == deregister_mr_array) {
        error_abort_all(GEN_EXIT_ERR,
                "dreg_init: unable to malloc %d bytes",
                (int) sizeof(struct ibv_mr*) *
                mvparams.ndreg_entries);
    }

    memset(deregister_mr_array, 0,
            sizeof(dreg_region) * mvparams.ndreg_entries);
    ...
    for(j = 0; j < n_dereg_mr; j++) {
        ...
        for(i = 0; i < npages; i++) {
            ...
            while( (d = dreg_lookup (begin, end)) != NULL) { 
                ...
                for(k = 0; k < mvdev.num_hcas; k++) {
                    if(d->memhandle[k]) {
                        if(ibv_dereg_mr(d->memhandle[k])) {
...
dreg_entry *dreg_new_entry(void *buf, int len, int acl)
{
    ...
    if ( mvparams.dreg_cache_limit != 0 && 
		  npages >= (int) mvparams.dreg_cache_limit ) {
        return NULL;
    }
    ...
    for(i = 0; i < mvdev.num_hcas; i++) {
        if(acl) {
            d->memhandle[i] = ibv_reg_mr(mvdev.hca[i].pd, (void *) pagebase_low_p, 
