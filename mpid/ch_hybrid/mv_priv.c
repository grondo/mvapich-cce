...
static int error_get_name(char* buf, int size)
{
     ...
     *   [rank X: host Y: pid Z] */
    if (mvdev.my_name != NULL && mvdev.pids != NULL && mvdev.me >= 0) {
        n = snprintf(buf, size, "[rank %d: host %s: pid %d]",
                mvdev.me, mvdev.my_name, mvdev.pids[mvdev.me]
        );
    } else if (mvdev.my_name != NULL) {
        n = snprintf(buf, size, "[rank %d: host %s]",
                mvdev.me, mvdev.my_name
        );
    } else if (mvdev.pids != NULL && mvdev.me >= 0) {
        n = snprintf(buf, size, "[rank %d: pid %d]",
                mvdev.me, mvdev.pids[mvdev.me]
        );
    } else {
