...
unsigned g_shmem_size = 0;
unsigned g_shmem_size_pool = 0;
                     ...
                     int context_id, int destination, MPIR_SHANDLE * shandle)
{
  volatile void *ptr_volatile;
    ...
    pkt->sequence_id = GET_NEXT_PKT(dest);
    INC_NEXT_PKT(dest);
...
smpi_post_send_ok_to_send (int destination, MPIR_RHANDLE * rhandle)
{
    ...
    if (!smpi.send_fifo_head
        ...
        pkt_p->sequence_id = GET_NEXT_PKT(dest);
        INC_NEXT_PKT(dest);
...
smpi_send_cancelok (void *in_pkt, int from)
{
    ...
    new_pkt->sequence_id = GET_NEXT_PKT(dest);
    INC_NEXT_PKT(dest);
     ...
     void *in_pkt;
{
    ...
    rhandle->protocol = MVDEV_PROTOCOL_SMP_RNDV;
...
smpi_init (void)
{
    ...
    sprintf (shmem_file, "/dev/shm/ib_shmem-%d-%s-%d.tmp", mvdev.global_id,
             mvdev.my_name, getuid ());

    sprintf (pool_file, "/dev/shm/ib_pool-%d-%s-%d.tmp", mvdev.global_id,
             mvdev.my_name, getuid ());
    ...
    if (smpi.fd < 0) {
        /* Fallback */
        sprintf (shmem_file, "/tmp/ib_shmem-%d-%s-%d.tmp", mvdev.global_id,
             mvdev.my_name, getuid ());

        sprintf (pool_file, "/tmp/ib_pool-%d-%s-%d.tmp", mvdev.global_id,
             mvdev.my_name, getuid ());
        smpi.fd =
            open (shmem_file, O_RDWR | O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO);
        if (smpi.fd < 0) {
    ...
    for (j = 1; j < smpi.num_local_nodes; j++) {
        ...
        while (SMPI_TOTALIN (from, smpi.my_local_id) !=
            ...
            if (pkt->head.sequence_id !=
                    mvdev.connections[global_rank].recv_window_start_seq) {
                break;
            }
            else {
                mvdev.connections[global_rank].recv_window_start_seq =
                    (mvdev.connections[global_rank].recv_window_start_seq + 1) % MAX_SEQ_NUM;
                if(0 == mvdev.connections[global_rank].recv_window_start_seq) {
                    mvdev.connections[global_rank].recv_window_start_seq++;
                }
            }
                            ...
                            MPID_Msgrep_t msgrep, MPIR_SHANDLE * shandle)
{
    ...
    if (!smpi.send_fifo_head &&
        ...
        pkt_ptr->tag = tag;
        pkt_ptr->len = len;
        pkt_ptr->sequence_id = GET_NEXT_PKT(dest);
        INC_NEXT_PKT(dest);
...
MPID_SMP_Eagerb_save_short (MPIR_RHANDLE * rhandle, int from, void *in_pkt)
{
    ...
    rhandle->protocol = MVDEV_PROTOCOL_SMP_SHORT;
    rhandle->connection = (void *) 1;
    rhandle->s.MPI_TAG = pkt->tag;
    rhandle->s.MPI_SOURCE = pkt->lrank;
    rhandle->s.MPI_ERROR = 0;
