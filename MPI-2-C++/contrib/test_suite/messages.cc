...
Testing(const char *message)
{
  ...
  if (indent_level == 0)
    std::cout << std::endl;
  std::cout << bullets[indent_level % num_bullets] << " " << message << "... ";
  std::cout.flush();
...
Abort(const char *msg)
{
  ...
  cerr << std::endl;
  if (msg != 0)
    cerr << "MPI2C++ test suite: " << msg << std::endl;

  cerr << "MPI2C++ test suite: major error!" << std::endl;
  cerr << "MPI2C++ test suite: attempting to abort..." << std::endl;
  ...
  if (my_rank <= 0)
    cerr << "MPI2C++ test suite: terminated" << std::endl << std::endl;
...
Endline(const char *msg)
{
  ...
  if (waiting) {
    ...
    std::cout << bullets[indent_level % num_bullets] << " " << line << "... ";
...
check_for_failures(int my_code, const char *msg)
{
  ...
  if (my_rank == 0) {
    ...
    if (num_fails == 1)
      ...
      cerr << "MPI2C++ test suite: attempting to determine which " << num_fails << " ranks failed..." << std::endl;
    else
      cerr << "MPI2C++ test suite: all ranks failed" << std::endl;
    
    // Was I one of the failures (this could only be rank 0)?
    
    if (my_code == 1 && num_fails < comm_size)
      cerr << "MPI2C++ test suite: rank 0 failed" << std::endl;
	...
	cerr << "MPI2C++ test suite: rank " << recv << " failed" << std::endl;
      MPI::COMM_WORLD.Recv(&emsg, sizeof(emsg), MPI::CHAR, MPI::ANY_SOURCE, 5678, status);
      if (num_fails < comm_size) {
	cerr << "MPI2C++ test suite: ERROR MESSAGE FOLLOWS " << std::endl;
	cerr << emsg << std::endl;
      }
    }
    
    cerr << "MPI2C++ test suite: minor error" << std::endl;
    cerr << "MPI2C++ test suite: attempting to finalize..." << std::endl;
  }
