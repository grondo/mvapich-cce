...
main(int argc, char *argv[])
{
  ...
  if (my_rank == 0)
    std::cout << std::endl
	 << "Since we made it this far, we will assume that" << std::endl
	 << "MPI::Init() worked properly." << std::endl 
	 << "----------------------------------------------" 
	 << std::endl 
	 << "MPI-2 C++ bindings test suite" << std::endl 
	 << "------------------------------" << std::endl 
	 << "LSC Version " 
	 << version[0] << "." << version[1] << std::endl 
	 << std::endl 
	 << "*** There are delays built into some of the tests" << std::endl 
	 << "*** Please let them complete" << std::endl 
	 << "*** No test should take more than 10 seconds" << std::endl << std::endl;
...
#define HANG 1
  ...
  if (my_rank == 0)
    std::cout << std::endl << "* MPI::Finalize..." << std::endl;
  ...
  if (my_rank == 0)
    std::cout << std::endl << std::endl
	 << "Since we made it this far, we will assume that" << std::endl
	 << "MPI::Finalize() did what we wanted it to." << std::endl
	 << "(Or, at the very least, it didn't fail.)" << std::endl
	 << "-----------------------------------------------------------" << std::endl
	 << "MPI-2 C++ bindings test suite: All done.  All tests passed." << std::endl
	 << std::endl;
  ...
  for (i = 1; i < argc; i++) {
    if (my_strcasecmp(argv[i], "-lam63") == 0)
      ...
      std::cout << "The following command line options are available:" << std::endl 
	   ...
	   << " -lam62       Skip tests for buggy LAM 6.2" << std::endl 
	   << " -lam63       Skip tests for buggy LAM 6.3.x" << std::endl 
	   << " -lam63       Skip tests for buggy LAM 6.4.x" << std::endl 
	   << " -ibm21014    Skip tests for buggy IBM SP MPI 2.1.0.14" << std::endl
	   << " -ibm21015    Skip tests for buggy IBM SP MPI 2.1.0.15" << std::endl
	   << " -ibm21016    Skip tests for buggy IBM SP MPI 2.1.0.16" << std::endl
	   << " -ibm21017    Skip tests for buggy IBM SP MPI 2.1.0.17" << std::endl
	   << " -ibm21018    Skip tests for buggy IBM SP MPI 2.1.0.18" << std::endl
	   << " -ibm2300     Skip tests for buggy IBM SP MPI 2.3.0.0" << std::endl
	   << " -sgi20       Skip tests for buggy SGI MPI 2.0" << std::endl
	   << " -sgi30       Skip tests for buggy SGI MPI 3.0" << std::endl
	   << " -sgi31       Skip tests for buggy SGI MPI 3.1" << std::endl
	   << " -sgi32       Skip tests for buggy SGI MPI 3.2" << std::endl
	   << " -hpux0102    Skip tests for buggy HP-UX MPI 1.02" << std::endl
	   << " -cray1104    Skip tests for buggy CRAY MPI 1.1.0.4" << std::endl
	   << " -nothrow     Skip exception tests for buggy compilers" << std::endl;
...
check_minimals()
{
  ...
  if (my_rank == 0)
    std::cout << "Test suite running on " << comm_size << " nodes" << std::endl;

  if (comm_size < 2) {
    if (my_rank == 0) {
      std::cout << "Sorry, the MPI2C++ test suite must be run with at least 2 processors" << std::endl;
      std::cout << "Please re-run the program with 2 or more processors." << 
	std::endl << std::endl;
    }
    MPI::Finalize();
    exit(1);
  ...
  if ((comm_size % 2) != 0) {
    if (my_rank == 0)
      std::cout << "The MPI2C++ test suite can only run on an even number" << std::endl 
	   << "of processors.  Please re-run the program with an even" << std::endl 
	   << "number of ranks." << std::endl << std::endl;
