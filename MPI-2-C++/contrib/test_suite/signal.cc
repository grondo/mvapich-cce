...
#endif
{
  ...
  std::cout << std::endl << std::endl << "MPI2C++ test suite (rank " << my_rank << "): ";
  if (block_me[i].number == -1)
    std::cout << "Unknown signal (" << sig << ") caught";
  else
    std::cout << block_me[i].name;
  std::cout << std::endl << "MPI2C++ test suite: aborting..." << std::endl;
  std::cout.flush();
