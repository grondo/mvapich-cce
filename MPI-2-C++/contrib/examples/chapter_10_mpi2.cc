...
example10_2()
{
  ...
  if (bar == MPI::COMM_NULL)
    std::cout << "bar is MPI::COMM_NULL" << std::endl;
}
...
example10_3()
{
  ...
  baz_comm = bar_comm;
  std::cout << "Is foocomm equal barcomm? " << (foo_comm == bar_comm) << std::endl;
  std::cout << "Is bazcomm equal barcomm? " << (baz_comm == bar_comm) << std::endl;
}
...
example10_5()
{
  ...
  if (comm == MPI::COMM_NULL)       // true
    std::cout << "comm is NULL" << std::endl;
  if (MPI::COMM_NULL == comm)       // note -- a different function
    std::cout << "comm is still NULL" << std::endl;
}
