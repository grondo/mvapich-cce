...
main(int argc, char *argv[])
{
  ...
  if (MPI::COMM_WORLD.Get_size() != 4) {
    if (my_rank == 0)
      cerr << "This program must be invoked with 4 ranks." << std::endl;
  } else {
    cartesian();
    graph();
  }
...
{
  if (my_rank == 0)
    std::cout << std::endl << "CARTESIAN TOPOLOGY" << std::endl << std::endl;
  ...
  std::cout << "cart is a " << dims[0] << "x" << dims[1] << " 2D mesh" << std::endl;
  ...
  std::cout << "Process " << my_rank << " is at coords: " << coords[0] << " "
    << coords[1] << "." << " To the left is node " 
      << left << ". to the right is node " << right << "." << std::endl;
}
...
graph()
{
  ...
  if (my_rank == 0)
    std::cout << std::endl << "GRAPH TOPOLOGY" << std::endl << std::endl;
