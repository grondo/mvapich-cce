...
# use the system debug for chaos5 and later
debugedit=/usr/lib/rpm/debugedit

# we use our own debug edit for chaos4
RH_RELEASE=$(rpm -qf /etc/redhat-release --qf %{VERSION} | colrm 2)
if [ "${RH_RELEASE}" == "5" ] ; then
  debugedit=./debugedit/debugedit
fi
...
#
for f in `find "${destdir}/${debugpath}" -type f \
...
rm $SOURCEFILE || die "rm $SOURCEFILE"

#
# Make sure directories and files are world readable.
#
chmod -R go+rX ${destdir}/${debugpath}/src  || die "chmod failed!"

#
# Make sure some sources at least were installed.
#
if [ $no_check = 0 ]; then
    checksource=${destdir}/${debugpath}/src/include/mpi.h
